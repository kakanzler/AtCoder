...
void solve() {
    ...
    ll current_type_count = color_count.size();

    ll idx = 0;
    rrep(i, m) {
        while (idx < (ll)events.size() && events[idx].first == i) {
            ll bird_idx = events[idx].second;
            ll A = a[bird_idx];
            ll B = b[bird_idx];
            ...
            if (color_count.find(B) == color_count.end()) {
                current_type_count++;
            }
            color_count[B]++;
