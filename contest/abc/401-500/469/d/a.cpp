...
void solve() {
    ...
    auto find_uncovered = [&](ll x) -> ll {
        rep(i, m) if (a[i] != x && b[i] != x) return i;
        return -1;
    };
    auto ok = [&](ll x, ll y) -> bl {
        rep(i, m) if (a[i]!=x && b[i]!=x && a[i]!=y && b[i]!=y) return false;
        return true;
    };

    set<P> ans;
    for (ll x : {a[0], b[0]}) {
        ll j = find_uncovered(x);
        if (j == -1) {
            rep(y, n) if (y != x) ans.em(min(x,y), max(x,y));
        } else {
            for (ll y : {a[j], b[j]}) {
                if (ok(x, y)) ans.em(min(x,y), max(x,y));
            }
        }
    }
