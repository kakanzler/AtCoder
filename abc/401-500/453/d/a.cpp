...
void solve() {
    ...
    // i: 現在の縦の座標, j:現在の横の座標, v: これから進む向き
    queue<tuple<int,int,int>> q;
    // i: 縦の座標, j:横の座標, 4方向: 過去に来たことがなければその向きに―1
    vector pre(h, vector(w, vector<int>(4, -1)));
    auto push = [&](int i, int j, int v, int pv){
        // 過去に呼ばれたことがある場合Queueにはいれない。
        // 未探索状態は -1
        if (pre[i][j][v] != -1) return;
        // 今回新しく入れるQueueであることをメモする。
        // 次のマス（i,j）について、どこ（v方向）から来たものがひとつ前はどこから(=pv)来たのか
        pre[i][j][v] = pv;
        // 次のマス(i,j)をqueueに入れる。
        q.emplace(i,j,v);
    };
    rep(i, h)rep(j, w) {
        if (grid[i][j] == 'S'){
            // スタートは4方向に動けるため、すべての状態(vが0~4の全方向)へ動けるように入れる。
            // pvにはありえない数字 9 をいれておく。
            // pvは 0: ↓ 1: ← 2: ↑ 3: →が入りうる。
    ...
    while (q.size()){
        ...
        // S or ・ の場合は4方向追加。
        rep(nv, 4) {
            if (grid[i][j] == 'o' && v != nv) continue;
            if (grid[i][j] == 'x' && v == nv) continue;
            int ni = i+di[nv];
            int nj = j+dj[nv];
